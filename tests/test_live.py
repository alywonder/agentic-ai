...
pytestmark = [
    pytest.mark.skipif(
        os.getenv('PYDANTIC_AI_LIVE_TEST_DANGEROUS') != 'CHARGE-ME!',
...
async def test_text(
    http_client: httpx.AsyncClient, tmp_path: Path, get_model: GetModel
) -> None:
    agent: Agent[None, str] = Agent(get_model(http_client, tmp_path))
...
async def test_structured(
    ...
    agent: Agent[None, MyModel] = Agent(
        get_model(http_client, tmp_path), result_type=MyModel
    )
